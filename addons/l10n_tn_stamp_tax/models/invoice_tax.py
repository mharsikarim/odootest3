...
class InvoiceStampTax(models.Model):
    ...
    stamp_tax = fields.Many2one('account.tax', string="Stamp Tax", domain=[('is_stamp', '=', True)], compute="compute_stamp_tax", readonly=True)
    fodec = fields.Many2one('account.tax',string="FODEC", domain=[('fodec', '=', True)])

    @api.depends('fiscal_position_id')
    def compute_stamp_tax(self):
        type_list = ['out_invoice', 'out_refund']
        type_list2 = ['in_invoice', 'in_refund']
        for rec in self:
            if rec.move_type in type_list:
                rec.stamp_tax = rec.fiscal_position_id.stamp_tax_id
            elif rec.move_type in type_list2:
                rec.stamp_tax = rec.fiscal_position_id.stamp_tax_id2
            else:
                rec.stamp_tax = False
    ...
    def _recompute_stamp_tax_lines(self):
        for rec in self:
            type_list = ['out_invoice', 'out_refund']
            type_list2 = ['in_invoice', 'in_refund']
            if rec.fiscal_position_id.stamp_tax_id and rec.move_type in type_list:
                tax = rec.fiscal_position_id.stamp_tax_id.compute_all(rec.amount_total, currency=rec.currency_id, quantity=1, product=None,
            ...
            elif not rec.fiscal_position_id.stamp_tax_id:
            ...
            elif rec.fiscal_position_id.stamp_tax_id2 and rec.move_type in type_list2:
                tax = rec.fiscal_position_id.stamp_tax_id2.compute_all(rec.amount_total, currency=rec.currency_id, quantity=1, product=None,
                                                partner=rec.partner_id)['taxes'][0]
                amount = tax['amount']
                if rec.is_invoice(include_receipts=True):
                    in_draft_mode = self != self._origin
            ...
            elif not rec.fiscal_position_id.stamp_tax_id2:
                stamp_lines = rec.line_ids.filtered(
                    lambda line: line.name and line.name.find('Droit de Timbre') == 0)
                if stamp_lines:
                    for line in stamp_lines:
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False, tax_rep_lines_to_recompute=False):
